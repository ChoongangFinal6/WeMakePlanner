...
<body>
	<div id="thisWeek">
		<div class='top'>
			<span class='topTitle'>한 주의 일정</span><span class='topDate'>~<fmt:formatDate value='${dateAfter}'
					pattern="yyyy년 MM월 dd일" /></span><br>
					<div id="expand"><a id="expandText">펼치기</a></div>
		</div>
		<div id="main">
			<c:forEach items="${thisWeek}" var="tw">
				<div class='item'>
					<div class='header'>
						<span class='title'>${tw.title}</span> <span class='dday'><c:if test="${tw.duration>0}">D-${tw.duration}</c:if></span>
					</div>
					<div class='main'>
						<span id="endTime${tw.no }">${tw.endTime} </span><br>
